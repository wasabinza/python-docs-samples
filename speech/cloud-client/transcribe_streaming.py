...
def transcribe_streaming(stream_file):
    ...
    def timingGenerator(audio_stream, chunkSize):
        from time import sleep
        from random import random
        finished = False
        while not finished:
            chunk = audio_stream.read(chunkSize)
            if random() > 1.9:
                chunk = audio_stream.read(chunkSize)

            if chunk:
                sleep(chunkSize/(sampleRate*bytesPerSample))
    ...
    with io.open(stream_file, 'rb') as audio_file:
        ...
        chunkSize = 32000
        requests = timingGenerator(audio_file, chunkSize)
